...
const getAnnouncements = async (req, res) => {
  const result = await pool.query(
    `SELECT a.*, u.name AS created_by_name, u.role AS creator_role FROM announcements a INNER JOIN users u ON a.created_by = u.id WHERE a.channel_id = $1 ORDER BY a.created_at DESC`,
    [req.params.id]
  );
...
const createAnnouncement = async (req, res) => {
  ...
  const result = await pool.query(
    `INSERT INTO announcements (channel_id,created_by,title,content,is_important) VALUES ($1,$2,$3,$4,$5) RETURNING *`,
