...
const getNotes = async (req, res) => {
  ...
  let query = `SELECT n.*, u.name AS uploaded_by_name FROM notes n INNER JOIN users u ON n.uploaded_by = u.id WHERE n.subject_id = $1`;
...
const uploadNote = async (req, res) => {
  ...
  const result = await pool.query(
    `INSERT INTO notes (subject_id, uploaded_by, title, description, file_url, file_name, file_type)
...
const deleteNote = async (req, res) => {
  const note = await pool.query('SELECT uploaded_by FROM notes WHERE id=$1', [req.params.id]);
