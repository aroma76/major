...
class AuthNotifier extends AsyncNotifier<AuthState> {
  ...
  Future<void> login(String identifier, String password) async {
    ...
    try {
      ...
      isLoggingIn = false;
      state =
          AsyncData(AuthState(status: AuthStatus.authenticated, user: user));
    } catch (e) {
      isLoggingIn = false;
  ...
  Future<void> updateProfile({String? name, String? dob}) async {
    ...
    if (current != null) {
      final merged = Map<String, dynamic>.from(current.user ?? {})
        ..addAll(updatedUser);
      state =
          AsyncData(AuthState(status: AuthStatus.authenticated, user: merged));
    }
  }
  ...
  String _parseError(Object? e) {
    if (e is DioException) {
      ...
      switch (e.response?.statusCode) {
        ...
        case 401:
          return 'Incorrect roll number / email or password.';
        case 404:
          return 'No account found with that Roll Number or Email.';
      }
